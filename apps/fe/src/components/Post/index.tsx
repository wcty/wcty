...
import { Button, Author, Text, Title } from "@ui";
    ...
    <Container onClick={() => options && setOptions(!options)} >
      ...
      <Actions>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <CommentCounter href={`/initiative/${initiative?.id}/post/${post_id}`}>
            <Title c='label' css={`font-size: 12px;`}><Trans>Comments:</Trans> {comments_aggregate?.aggregate?.count}</Title>
          </CommentCounter>
          <Button
            onClick={() => {
              router.push({
