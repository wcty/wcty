  ...
  useEffect(() => {
    setTimeout(async () => {
      const features = await waitForFeatures(
        () =>
          map?.getLayer('entries') &&
          map?.queryRenderedFeatures(undefined, { layers: ['entries'] })
      );
      if (features)
        startTransition(() =>
    ...
    <>
      {isEntryCreation && focus ? (
        ...
        <>
            ...
            {map.getSource('entries-clusters') && (
              <>
                <Layer
                  ...
                  onEnter={(e: any) => {
                    setHovered(e?.features?.[0]?.properties?.cluster_id);
                    setCursor('pointer');
                  }}
                  onLeave={(e: any) => {
                    setHovered('');
          ...
          {map.getSource('russia-is-a-terrorist-state-label') && (
            <>
              <Layer
                source="russia-is-a-terrorist-state-label"
                ...
                layout={{
                  'text-field': 'RUSSIA IS A TERRORIST STATE',
                  'text-font': ['Montserrat SemiBold'],
                }}
              />
            </>
