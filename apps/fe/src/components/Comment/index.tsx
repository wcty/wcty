...
import {
  ...
  CommentFragment,
  SubCommentFragment,
  GetFilesWithStatsDocument,
  PostInitiativeInfoFragment
} from "generated";
...
export default function Comment({ comment, onReply, initiative }:
  {
  ...
  return (<>
    <Container onClick={() => options && setOptions(!options)} >
      ...
      <Content>
        {editorOpen ?
          <CommentCreation initiative={initiative} noAvatar ml='-2rem' width='calc(100% + 4rem)' comment={comment} onClose={() => setEditorOpen(false)} /> :
          <Message>{
            message?.replaceAll(
              '\\n', `
          `)
      ...
      {!comment?.parent_comment_id &&
        ...
        <EditorContainer>
          {comment?.comments.map((c, key) =>
            <Comment initiative={initiative} key={key} {...{ comment: c, onReply: () => setReplyOpen(true) }} />)}
          {replyOpen && <CommentCreation initiative={initiative} parent={comment} />}
        </EditorContainer>}
    </Container>
    {fullscreen &&
      <FullscreenCarousel
